...
impl<'c> OpCollection<'c> {
    pub fn new(block_id: BlockId, block: Block<'c>) -> Self {
        Self {
            block_id,
            op_count: 0,
            block: Some(block),
...
pub struct MLIRGenerator<'c> {
    ...
    blocks: HashMap<BlockId, OpCollection<'c>>,
...
impl<'c> MLIRGenerator<'c> {
    ...
    pub fn ensure_call_args_empty(&self) {
        assert_eq!(self.call_args.len(), 0);
    }

    pub fn take_block(&mut self, block_id: BlockId) -> Block<'c> {
...
impl<'c> MLIRGenerator<'c> {
    ...
    pub fn lower_jump(&mut self, link_id: LinkId, target: BlockId) -> Result<()> {
        ...
        let c = self
            .blocks
            .get(&target)
            .expect(&format!("missing block at {}", target));
    ...
    pub fn lower_switch(
        ...
        let indicies = values
            .into_iter()
            .map(|call_value_id| {
                self.resolve_value_lower_load(block_id, call_value_id)
                    .unwrap()
            })
            .collect();
        let rs = self.values(indicies);
        ...
        let case_destinations = case_values
            .iter()
            .map(|i| {
                let target_block_id = m.get(&(*i as usize)).unwrap();
                //let target_value_id = self.blockify.value(block_id);
                let c = self
                    .blocks
                    .get(&target_block_id)
                    .expect(&format!("missing block at {}", target_block_id));
                let arg_count = c.block.as_ref().unwrap().argument_count();
                assert_eq!(
                    arg_count,
                    arity,
    ...
    pub fn build_struct_type(&mut self, values: Vec<LinkId>) -> (Type<'c>, Type<'c>) {
        let types = values
            .iter()
            .map(|v| {
                let ast_ty = self.blockify.get_type(v);
    ...
    */
    ...
    fn lower_store(&mut self, link_id: LinkId, v_decl: LinkId, v_value: LinkId) -> SymIndex {
        ...
        let addr_index = if decl_is_static {
            ...
            let lhs_ty = self.blockify.get_type(v_decl);
    ...
    pub fn lower_code(&mut self, link_id: LinkId) -> Result<()> {
        ...
        match code {
            LCode::Label => {
                ...
                let block_id = self.blockify.get_block_id(link_id);
                assert!(self.blocks.get(&block_id).is_some());
            }
            ...
            LCode::DeclareFunction(maybe_block_id) => {
                ...
                let ty = self.blockify.get_type(link_id);
                ...
                if let Some(block_id) = maybe_block_id.clone() {
                    ...
                    for block_id in block_ids.iter() {
                        self.append_op(index, *block_id, 0);
                    }
                }
            ...
            LCode::Call(v_f) => {
                ...
                let ty = self.blockify.get_type(v_f);
            ...
            LCode::Declare => {
                self.ensure_call_args_empty();
                let block_id = self.blockify.get_block_id(link_id);
                let ast_ty = self.blockify.get_type(link_id);
                let (ty, dims) = self.from_type(&ast_ty);
            ...
            LCode::Op2(op) => {
                ...
                let x_index = self.resolve_value_lower_load(block_id, vx).unwrap();
                let y_index = self.resolve_value_lower_load(block_id, vy).unwrap();
            ...
            LCode::Branch(condition, then_block_id, else_block_id) => {
                self.ensure_call_args_empty();
                let block_id = self.blockify.get_block_id(link_id);
                let c_index = self.resolve_value_lower_load(block_id, *condition).unwrap();
                let r_c = self.value0(c_index);

                let c = self.blocks.get(then_block_id).unwrap();
                let then_block = c.block.as_ref().unwrap();

                let c = self
                    .blocks
                    .get(else_block_id)
                    .expect(&format!("missing else block in branch: {}", else_block_id));
            ...
            LCode::Ternary(condition, then_block_id, else_block_id) => {
                ...
                let c = self.blocks.get_mut(&then_block_id).unwrap();
                ...
                // yield the last value
                let c = self.blocks.get_mut(&else_block_id).unwrap();
                let r: Value<'c, '_> = c.ops.last().unwrap().operand(0).unwrap().into();
                let else_ty = r.r#type();

                let then_region = Region::new();
                for block_id in then_block_ids.iter() {
                    let block = self.take_block(*block_id);
                ...
                match bi {
                    ...
                    Builtin::Assert => {
                        let indicies = values
                            .into_iter()
                            .map(|value_id| {
                                self.resolve_value_lower_load(block_id, value_id).unwrap()
                            })
                            .collect();
                        let rs = self.values(indicies);
    ...
    pub fn lower_static_block(&mut self) -> Result<()> {
        ...
        self.blocks.get_mut(&static_block_id).unwrap().complete = true;
        Ok(())
    }
    ...
    pub fn lower_module(&mut self, module: &mut melior::ir::Module) -> Result<()> {
        ...
        let static_block_id = self.blockify.blocks.static_block_id();
        let block = self.blocks.get_mut(&static_block_id).unwrap();
