...
use compile_core::{SpanId, SpannedError as TypeError};
...
fn check_heads(
    lhs_ind: ID,
    lhs: &(VTypeHead, SpanId),
    rhs_ind: ID,
    rhs: &(UTypeHead, SpanId),
...
enum TypeNode {
    Var,
    Value((VTypeHead, SpanId)),
    Use((UTypeHead, SpanId)),
}
#[derive(Debug, Clone)]
...
impl TypeCheckerCore {
    ...
    fn new_val(&mut self, val_type: VTypeHead, span: SpanId) -> Value {
    ...
    fn new_use(&mut self, constraint: UTypeHead, span: SpanId) -> Use {
        let i = self.r.add_node();
        assert!(i == self.types.len());
    ...
    pub fn bool(&mut self, span: SpanId) -> Value {
        self.new_val(VTypeHead::VBool, span)
    }
    pub fn float(&mut self, span: SpanId) -> Value {
        self.new_val(VTypeHead::VFloat, span)
    }
    pub fn int(&mut self, span: SpanId) -> Value {
        self.new_val(VTypeHead::VInt, span)
    }
    pub fn null(&mut self, span: SpanId) -> Value {
        self.new_val(VTypeHead::VNull, span)
    }
    pub fn str(&mut self, span: SpanId) -> Value {
        self.new_val(VTypeHead::VStr, span)
    }

    pub fn bool_use(&mut self, span: SpanId) -> Use {
        self.new_use(UTypeHead::UBool, span)
    }
    pub fn float_use(&mut self, span: SpanId) -> Use {
        self.new_use(UTypeHead::UFloat, span)
    }
    pub fn int_use(&mut self, span: SpanId) -> Use {
        self.new_use(UTypeHead::UInt, span)
    }
    pub fn null_use(&mut self, span: SpanId) -> Use {
        self.new_use(UTypeHead::UNull, span)
    }
    pub fn str_use(&mut self, span: SpanId) -> Use {
        self.new_use(UTypeHead::UStr, span)
    }
    pub fn int_or_float_use(&mut self, span: SpanId) -> Use {
        self.new_use(UTypeHead::UIntOrFloat, span)
    }

    pub fn func(&mut self, arg: Use, ret: Value, span: SpanId) -> Value {
        self.new_val(VTypeHead::VFunc { arg, ret }, span)
    }
    pub fn func_use(&mut self, arg: Value, ret: Use, span: SpanId) -> Use {
    ...
    pub fn obj_use(&mut self, field: (String, Use), span: SpanId) -> Use {
        self.new_use(UTypeHead::UObj { field }, span)
    }

    pub fn case(&mut self, case: (String, Value), span: SpanId) -> Value {
    ...
    pub fn reference(&mut self, write: Option<Use>, read: Option<Value>, span: SpanId) -> Value {
        self.new_val(VTypeHead::VRef { write, read }, span)
    }
    pub fn reference_use(&mut self, write: Option<Value>, read: Option<Use>, span: SpanId) -> Use {
        self.new_use(UTypeHead::URef { write, read }, span)
    }

    pub fn null_check_use(&mut self, nonnull: Use, span: SpanId) -> Use {
